...
for i = 1:length(algorithms)
    ...
    load(['F:\Onedrive\Experiment\Test\2EVRPLDTW-6\Cov\',algo_name,'_Ca1_35_100' ...
        '_TW_CV.mat'])
    % if i<=2
    %     A(i,:) = HV_value;
    % else
...
for i = 1:6
    plot(generations, HV_values(:, i), ...
        'Color', colors(i, :),'LineWidth', 1.2); %,'Marker', markers{i},'MarkerIndices', 1:5:100,'MarkerSize', 6, 'MarkerFaceColor', colors(i, :)
        % 线条颜色           % 线宽               % 点形状       % 每隔10代显示一个点    % 点大小   % 点填充颜色
end

% 3. 添加图例和标签
legend('LNS','ALNS','CMOEAD','DNNSGAII','SFEA','DSDE', ...
    'Location', 'northwest', 'FontSize', 10);
xlabel('Iteration ratio', 'FontSize', 12);
ylabel('Constraints violation (CV)', 'FontSize', 12);
