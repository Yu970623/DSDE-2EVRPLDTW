...
Data = cell(60,6);  
time_compare = zeros(60,6);
for i = 1:2  % 算法
    HV_sigle = zeros(1,8);
    for j = 1:60  % 注意：现在遍历 1..60，靠 row 映射到 1..60 行
        ...
        for k = 1:8
            ...
            Pop_ND = nd_objs{k};
            if isnan(HV_out(Pop_ND, nadir_point(ceil(j/15),:)))
                HV_sigle(1,k) = 0;
            else
                HV_sigle(1,k) = HV_out(Pop_ND, nadir_point(ceil(j/15),:));
            end
            
        end
        filename1 = ['F:\Onedrive\Experiment\', algorithm_name{i}, '\vrp_data\', ...
                        algorithm_name{i}, '_Set1_num', num2str(j), '.mat'];
        load(filename1)
        time_compare(row, i) = cpu_time;
        Data{row, i} = HV_sigle;
    end
end
DS  = {'23','35','64'};
        ...
        for j = 1:5         % Ca1~Ca5
            for k = 1:numel(DS)  % 仓库-卫星规模 23/35/64
                ...
                for n = 1:8  % 运行次数
                    ...
                    HV_sigle(1,n) = HV_out(result{1,2}.objs, nadir_point(ceil(row/15),:));
                    time_single(1,n) = metric.runtime;
                end
                Data{row, i} = HV_sigle;
                time_compare(row, i) = min(time_single);
            end
        end
    end
end
% xlswrite('Time_6.xlsx', time_compare);
% 计算每个问题上各算法的均值、标准差及比较结果
results = cell(61, 7);  % 61行，9列（包括表头）
...
% 初始化统计结果
stats = zeros(6, 3);  % 每个算法的统计结果，4行（算法数），3列（领先、弱于、近似）
% 逐问题处理
...
for i = 1:60
    ...
    comp_1 = compare(Data{1,i}, Data{6,i}, epsilon);
    comp_2 = compare(Data{2,i}, Data{6,i}, epsilon);
    comp_3 = compare(Data{3,i}, Data{6,i}, epsilon);
    comp_4 = compare(Data{4,i}, Data{6,i}, epsilon);
    comp_5 = compare(Data{5,i}, Data{6,i}, epsilon);
    % 存储计算结果
    ...
    results{i+1, 5} = sprintf('%.4e (%.4e) %s', mean(Data{4,i}), std(Data{4,i}), comp_4);
    results{i+1, 6} = sprintf('%.4e (%.4e) %s', mean(Data{5,i}), std(Data{5,i}), comp_5);
    results{i+1, 7} = sprintf('%.4e (%.4e) ', mean(Data{6,i}), std(Data{6,i}));  % CRCEA与自己比较，始终为"="
    ...
    if strcmp(comp_4, '+')
        stats(4, 1) = stats(4, 1) + 1;  % DNNSGAII领先
    elseif strcmp(comp_4, '-')
        stats(4, 2) = stats(4, 2) + 1;  % DNNSGAII弱于
    else
        stats(4, 3) = stats(4, 3) + 1;  % DNNSGAII近似
    end
    if strcmp(comp_5, '+')
        stats(5, 1) = stats(5, 1) + 1;  % SFEA领先
    elseif strcmp(comp_5, '-')
        stats(5, 2) = stats(5, 2) + 1;  % SFEA弱于
    else
        stats(5, 3) = stats(5, 3) + 1;  % SFEA近似
    end
end
% 添加统计结果行到表格
results{62, 1} = '+,-,=';  % 第一列：'+,-,='
...
results{62, 5} = sprintf('%d/%d/%d', stats(4, 1), stats(4, 2), stats(4, 3));  % DNNSGAII统计结果
results{62, 6} = sprintf('%d/%d/%d', stats(5, 1), stats(5, 2), stats(5, 3));  % SFEA统计结果

% 将结果写入Excel文件
xlswrite('Compare_6.xlsx', results);
