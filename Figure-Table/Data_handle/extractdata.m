%% --------------------------提取源文件中的数据---------------------
function [vrp2e,timewindows,timeservice,demand,type,dis_ds,dis_sc,num_dep,num_sat,num_cus] = extractdata(set,seq)
    ...
    filename     = ['D:\OneDrive\Yu\OneDrive\Experiment\Test\2EVRPLDTW-6\Instance\',case_name,'.csv'];
    % filename     = ['/Users/yufan/Desktop/outside/PlatEMO-VRPLDTW/Instance/',case_name,'.csv']; 
    disp([case_name,' read']);
    
    % 读取CSV文件，指定第一行为表头，并跳过第一行
    table_data = readtable(filename, 'HeaderLines', 1);
    
    % 提取需要的列
    coords = table_data{:, 1:2};  % 第一列和第二列是坐标信息
    attributes = table_data{:, 7};  % 第七列是点的属性
    demands = table_data{:, 5};  % 第五列是货物需求量
    customer_types = table_data{:, 10};  % 第十列是客户属性，取-送
    times = table_data{:, 3:4};
    service = table_data{:, 6};
...
vrp2e.fleet = fleet;
vrp2e.timewindows = timewindows;
vrp2e.timeservice = timeservice;
vrp2e.demand = demand;
vrp2e.type = type;
vrp2e.dis_ds = dis_ds;
vrp2e.dis_sc = dis_sc;
