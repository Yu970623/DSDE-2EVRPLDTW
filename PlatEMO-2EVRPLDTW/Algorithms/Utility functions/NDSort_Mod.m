function [FrontNo,MaxFNo] = NDSort_Mod(varargin)
...
%--------------------------------------------------------------------------
    ...
    if nargin == 4
        nSort  = varargin{2};
        PopDec = varargin{3};
        R = varargin{4};    % radius supporting domination relationship
    else
        PopCon = varargin{2};
        nSort  = varargin{3};
        PopDec = varargin{4};
    ...
    if M < 3 || N < 500
        ...
        [FrontNo,MaxFNo] = ENS_SS(PopObj,nSort,PopDec,R);
    else
...
% ---------------------改动：考虑距离的非支配排序
function [FrontNo,MaxFNo] = ENS_SS(PopObj,nSort,PopDec,R)
    ...
    FrontNo = inf(1,N);
    MaxFNo  = 0;
    while sum(FrontNo<inf) < nSort
        MaxFNo = MaxFNo + 1;
        for i = 1 : N
            if FrontNo(i) == inf
                Dominated = false;
                for j = i-1 : -1 : 1    % 当前个体只有可能被前面的个体支配，因为unique升序排列
                    if FrontNo(j) == MaxFNo
                        % ----------------改动：在邻域中的个体产生支配关系
                        if distance(i,j)>R || sum(PopObj(i,:)-PopObj(j,:)==0)==M
                            continue;
                        end
                        m = 2;
                        ...
                        Dominated = m > M;  %distance(i,j)<R && 
                        if Dominated || M == 2  % 2维的情况，被支配或者不被支配
                            break;
                        end
                    end
                end
